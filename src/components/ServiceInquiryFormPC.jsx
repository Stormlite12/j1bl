import { useState } from "react";

function ServiceInquiryFormPC(props) {
  const [selectedServices, setSelectedServices] = useState([]);
  const { service, setService } = props;
  const services = [
    "Strategy Development",
    "Branding & Imagery",
    "Email Marketing",
    "Social Media Adverts.",
    "Social Media Marketing",
  ];

  const handleSelection = (service) => {
    setSelectedServices((prev) =>
      prev.includes(service)
        ? prev.filter((s) => s !== service) // Unselect if already selected
        : [...prev, service] // Add if not selected
    );
    setService((prev) =>
      prev.some((s) => s.value === service)
        ? prev.filter((s) => s.value !== service) // Unselect if already selected
        : [...prev, { value: service }] // Add if not selected
    );
  };

  return (
    <div className="">
      <div className="grid grid-cols-2 gap-6 lg:gap-7 xl:gap-8 py-4 lg:py-5 xl:py-6">
        {services.map((service, index) => (
          <label
            key={index}
            className="flex items-center space-x-2 cursor-pointer"
            onClick={() => handleSelection(service)}
          >
            <span className="cursor-pointer w-10 h-10 flex items-center justify-center">
              {selectedServices.includes(service) ? (
                <svg
                  width="28"
                  height="28"
                  viewBox="0 0 28 28"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M14 28C11.2311 28 8.52431 27.1789 6.22202 25.6406C3.91973 24.1022 2.12532 21.9157 1.06569 19.3576C0.00606596 16.7994 -0.271181 13.9845 0.269012 11.2687C0.809205 8.55301 2.14258 6.05845 4.10051 4.10051C6.05845 2.14258 8.55301 0.809205 11.2687 0.269012C13.9845 -0.271181 16.7994 0.00606596 19.3576 1.06569C21.9157 2.12532 24.1022 3.91973 25.6406 6.22202C27.1789 8.52431 28 11.2311 28 14C28 17.713 26.525 21.274 23.8995 23.8995C21.274 26.525 17.713 28 14 28ZM14 2.00001C11.6266 2.00001 9.30655 2.70379 7.33316 4.02237C5.35977 5.34095 3.8217 7.21509 2.91345 9.40781C2.0052 11.6005 1.76756 14.0133 2.23058 16.3411C2.69361 18.6689 3.83649 20.8071 5.51472 22.4853C7.19296 24.1635 9.33115 25.3064 11.6589 25.7694C13.9867 26.2325 16.3995 25.9948 18.5922 25.0866C20.7849 24.1783 22.6591 22.6402 23.9776 20.6668C25.2962 18.6935 26 16.3734 26 14C26 10.8174 24.7357 7.76516 22.4853 5.51472C20.2348 3.26429 17.1826 2.00001 14 2.00001ZM12.66 18.75L21.66 10.75L20.34 9.25001L12 16.63L7.71001 12.29L6.29001 13.71L11.29 18.71C11.3834 18.8027 11.4943 18.876 11.6161 18.9258C11.7379 18.9755 11.8684 19.0008 12 19C12.2431 18.9997 12.4777 18.9108 12.66 18.75Z"
                    fill="#545454"
                  />
                </svg>
              ) : (
                // Unticked SVG
                <svg
                  width="28"
                  height="28"
                  viewBox="0 0 28 28"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <circle
                    cx="14"
                    cy="14"
                    r="12"
                    stroke="#545454"
                    strokeWidth="2"
                  />
                </svg>
              )}
            </span>
            <span className="text-[#535353] text-sm lg:text-base xl:text-lg 2xl:text-xl font-medium font-title leading-loose tracking-tight">
              {service}
            </span>
          </label>
        ))}
      </div>
    </div>
  );
}

export default ServiceInquiryFormPC;